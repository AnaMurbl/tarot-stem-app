...
function Card({ card, isRevealed = false, onClick, disableNavigation = false }) {
  const cardContent = (
  ...
  // Si tiene onClick o navigation está deshabilitada, usar div clickeable
  if (onClick || disableNavigation) {
    return (
      <div 
        className="w-full max-w-xs mx-auto cursor-pointer transition-all duration-300 hover:scale-105"
  ...
  // Si no, usar Link para navegación normal
  return (
    <Link 
      to={`/card/${card.id}`}
